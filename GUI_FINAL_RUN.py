...
def serverIP():
    window = Tk()
    window.title("Criminal Detection | Server IP")
    w = 1280
    h = 720
    ...
    def confirm():
        global BASE
        if entServerIP.get() == "":
            messagebox.showwarning(title = "Warning", message = "Please enter Server IP to Proceed.")
        else:
            BASE = entServerIP.get()
            window.destroy()
            login()
    ...
    lblText = Label(master=frame, text='Server IP Config', font=('roboto', 32), bg='#939393').pack(padx=5, pady=5)

    frame = Frame(master=window, bg="#C4C4C4")
    frame.grid(row=1, column=1, pady=5)
    lblServerIP = Label(master=frame, text='Enter Server IP:', font=('roboto', 24), bg='#C4C4C4')
    lblServerIP.pack(padx=9, pady=5)

    frame = Frame(master=window, bg="#C4C4C4")
    frame.grid(row=1, column=2, pady=5)
    entServerIP = Entry(master=frame, font=('roboto', 25), width=30, bg='#81D6E1')
    entServerIP.pack(padx=5, pady=5)
...
def forgotPassword():
    ...
    def confirm():
        ...
        if entRecovery.get() == "" or entPassword.get() == "":
            messagebox.showwarning(title = "Warning", message = "Please enter Recovery code or New password")
        else:
            response = requests.put(BASE + "ForgotPassword",{"recovery_code": entRecovery.get(), "pass": entPassword.get()})
            print(response.json())
            if response.json() == "recovery code is valid" :
                window.destroy()
                login()
            elif response.json()== "recovery code is not valid":
...
def photoBrowser():
    ...
    def browseImg():
        ...
        file = {('frame.jpg', open('frame.jpg', 'rb'), 'image/jpg'), }
        response = requests.put(BASE + "DetectByPhoto", {"cphoto": file})

        if response.json()=="match not found":
            messagebox.showwarning("Alert", "Criminal not found in database")
        else:
            st =response.json()
            name=st[0]+" "+st[1]+" "+st[2]
            entName.configure(state="normal")
            entRecord.configure(state="normal")
            entName.delete(0,END)
            entRecord.delete(0, END)
            entName.insert(0,name)
            entRecord.insert(0, st[3])
            entName.configure(state="disabled")
            entRecord.configure(state="disabled")
            ...
            finalImgPIL = finalImgPIL.resize((300, 300))
...
def liveFeed():
    ...
    def alert(name, record, location, detectedImage):
        window1 = Toplevel()
        window1.title("Criminal Detection | Alert")
        w = 1280
        h = 720
        ...
        window1.iconbitmap("imagesIcons/CDLogo.ico")
        window1.config(bg="#939393")

        # tmpImage = Image.open("frame.jpg")
        detectedImage = cv2.cvtColor(detectedImage, cv2.COLOR_BGR2RGB)
        tmpImage = Image.fromarray(detectedImage)
        tmpImage = tmpImage.resize((500, 400))
        tmpImage = ImageTk.PhotoImage(tmpImage)

        frmSpace = Frame(master=window1, bg="#939393")
        ...
        frmLeft = Frame(master=window1, bg="#939393")
        frmLeft.grid(row=1, column=1)

        lblPhoto = Label(master=frmLeft, bd=0)
        lblPhoto.tmpImage = tmpImage
        lblPhoto.configure(image=tmpImage)
        lblPhoto.pack(padx=70, pady=1)

        frmRight = Frame(master=window1, bg="#939393", height=400, width=500)
        frmRight.grid(row=1, column=2)
        ...
        frame = Frame(master=frmRight, bg="#C4C4C4")
        frame.grid(row=1, column=0, pady=5)
        entName = Entry(master=frame, font=('roboto', 25), width=30, bg='#81D6E1', state="disabled")
        entName.pack(padx=5, pady=5)
        ...
        frame = Frame(master=frmRight, bg="#C4C4C4")
        frame.grid(row=3, column=0, pady=5)
        entRecord = Entry(master=frame, font=('roboto', 25), width=30, bg='#81D6E1', state="disabled")
        entRecord.pack(padx=5, pady=5)
        ...
        frame = Frame(master=frmRight, bg="#C4C4C4")
        frame.grid(row=5, column=0, pady=5)
        entLocation = Entry(master=frame, font=('roboto', 25), width=30, bg='#81D6E1', state="disabled")
        entLocation.pack(padx=5, pady=5)

        frmBottom = Frame(master=window1, bg="#939393", height=100, width=100)
        frmBottom.grid(row=2, column=0, columnspan=3, pady=10)

        frame = Frame(master=frmBottom, bg="#C4C4C4")
        frame.grid(row=4, column=0, pady=5)
        btnOk = Button(master=frame, text='Ok', font=('roboto', 25), width=30, bd=5, bg='#81D6E1', command=window1.destroy)
        btnOk.pack(padx=5, pady=5)

        entName.configure(state="normal")
        entRecord.configure(state="normal")
        entLocation.configure(state="normal")
        entName.insert(0, name)
        entRecord.insert(0, record)
        entLocation.insert(0, location)
        entName.configure(state="disabled")
        entRecord.configure(state="disabled")
        entLocation.configure(state="disabled")

    def detected(response, location, detectedImage):
        global alertedID
        mylist = response.json()
        id = mylist[0][0]
        name = mylist[0][1] + " " + mylist[0][2] + " " + mylist[0][3]
        record = mylist[0][4]
        flag = 0
        for i in alertedID:
            if i==id:
        ...
        if flag == 0:
            alertedID.append(id)
            t1 = threading.Thread(target=alert, args=(name, record, location, detectedImage))
            t1.start()
        elif flag > 0:
            pass
    ...
    def showFrame():
        global counter
        global list, list1, list2, list3, list4
        counter+=1
        if cameraNo == "1":
            _, frame1 = cap0.read()
            imgS = frame1
            ...
            if mode == "Webcam":
                ...
                location = presetData[0]["ip1"]

            if counter%10 == 0:
                cv2.imwrite("frame.jpg", imgS)
                file = {('frame.jpg', open('frame.jpg', 'rb'), 'image/jpg'), }
                try:
                    response = requests.put(BASE + "LiveDetection", {"curimg": file})
                    if response.json() != "match not found":
                        list=response.json()
                        if list[1][0]:
                            d=list[1][0]
                            ...
                            detectedImage = frame1

                        detected(response, location, detectedImage)
                    elif response.json() == "match not found":
                        list = []
                except Exception:
                    pass
            else:
            ...
            _, frame0 = cap0.read()
            imgS0 = frame0
            imgS0 = cv2.resize(imgS0, (0, 0), None, 0.25, 0.25)

            _, frame1 = cap1.read()
            imgS1 = frame1
            imgS1 = cv2.resize(imgS1, (0, 0), None, 0.25, 0.25)

            if counter%10==0:
                for i in range(2):
                    if i == 0:
                        cv2.imwrite("frame.jpg", imgS0)
                    elif i == 1:
                        cv2.imwrite("frame.jpg", imgS1)
                    file = {('frame.jpg', open('frame.jpg', 'rb'), 'image/jpg'), }
                    try:
                        response = requests.put(BASE + "LiveDetection", {"curimg": file})
                        if response.json() != "match not found":
            ...
            _, frame2 = cap2.read()
            imgS2 = frame2
            imgS2 = cv2.resize(imgS2, (0, 0), None, 0.25, 0.25)
                            ...
                            if list[1][0]:
                                ...
                                if i == 0:
                                    ...
                                    if mode == "Webcam":
                                        ...
                                        location = presetData[0]["ip3"]
                                    detectedImage = frame2
                            detected(response, location, detectedImage)
                        elif response.json() == "match not found":
                            if i == 0:
                                list1 = []
            ...
            _, frame3 = cap3.read()
            imgS3 = frame3
            imgS3 = cv2.resize(imgS3, (0, 0), None, 0.25, 0.25)
                            ...
                            if list[1][0]:
                                ...
                                if i == 0:
                                    ...
                                    if mode == "Webcam":
                                        ...
                                        location = presetData[0]["ip4"]
                                    detectedImage = frame3
                            detected(response, location, detectedImage)
                        elif response.json() == "match not found":
                            if i == 0:
                                list1 = []
