...
def photoBrowser():
    ...
    def browseImg():
        ...
        img = {(imgPath, open(imgPath, 'rb'), 'image/jpg'), }
        response = requests.put(BASE + "DetectByPhoto", {"cphoto": img})
...
def liveFeed():
    ...
    def detected(response):
        ...
        list = response.json()
        id = list[0][0]
        name = list[0][1] + " " + list[0][2] + " " + list[0][3]
        record = list[0][4]
        ...
        for i in alertedID:
            if i==id:
                ...
                break
        if flag == 0:
            alert(name, record, id)
        elif flag > 0:
            print("Criminal already found")
...
def alert(name, record, id):
    ...
    #tmpImage = {('frame.jpg', open('frame.jpg', 'rb'), 'image/jpg'), }
    tmpImage =  ImageTk.PhotoImage(Image.open("frame.jpg"))
    ...
    frame = Frame(master=frmRight, bg="#C4C4C4")
    frame.grid(row=1, column=0, pady=5)
    entName = Entry(master=frame, font=('roboto', 25), text=name, width=30, bd=0, disabledbackground='#939393', state="disabled")
    entName.pack()
    ...
    frame = Frame(master=frmRight, bg="#C4C4C4")
    frame.grid(row=3, column=0, pady=5)
    entRecord = Entry(master=frame, font=('roboto', 25), text=record, width=30, bd=0, disabledbackground='#939393', state="disabled")
    entRecord.pack()
    ...
    frame.grid(row=4, column=0, pady=5)
    btnContinue = Button(master=frame, text='Continue', font=('roboto', 25), width=30, bd=5, bg='#81D6E1', command=window.destroy)
